...
def baseline_main(
    ...
    shap_values = shap_values[:, :, 1]  # positive class predictions
    shap_values = pd.DataFrame(shap_values, columns=feature_names)
    vals = np.abs(shap_values.values).mean(0)
    shap_importance = pd.DataFrame(list(zip(feature_names, vals)), columns=['col_name', 'feature_importance_vals'])
    shap_importance.sort_values(by=['feature_importance_vals'], ascending=False, inplace=True)

    zip_fi = list(zip(shap_importance['col_name'].values, shap_importance['feature_importance_vals'].values))
...
if __name__ == "__main__":
    ...
    for openml_id in BENCHMARK_DATASETS:
        ...
        if os.path.isfile('./plots/RF_SHAP_single_deletion.csv'):
            current_results = pd.read_csv('./plots/RF_SHAP_single_deletion.csv')
        ...
        if args.model_name == "RF":
            ...
            if os.path.isfile('./plots/RF_SHAP_single_deletion.csv'):
                single_deletion_results.to_csv('./plots/RF_SHAP_single_deletion.csv', index=False, header=False, mode='a+')
            else:
                single_deletion_results.to_csv('./plots/RF_SHAP_single_deletion.csv', index=False)
        else:
