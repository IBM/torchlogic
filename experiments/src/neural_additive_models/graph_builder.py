...
def roc_auc_score(sess, y_true, pred_tensor,
                  dataset_init_op, print_size=False):
  ...
  if print_size:
    print("SIZES", len(y_true), len(y_pred))
  return sk_metrics.roc_auc_score(y_true, y_pred, multi_class='ovo', average='micro')
...
def create_balanced_dataset(x_train, y_train,
  ...
  def partition_dataset(
    ...
    neg_mask = (y_train == 0)
    x_train_neg = x_train[neg_mask.ravel()]
    y_train_neg = np.zeros(len(x_train_neg), dtype=np.float32)
    x_train_pos = x_train[~neg_mask.ravel()]
...
def build_graph(
  ...
  x_batch, (train_init_op, test_init_op) = create_iterators((x_train, x_test),
                                                            batch_size)
  if x_true_test is not None and y_true_test is not None:
      true_test_x_batch, (_, true_test_init_op) = create_iterators((x_train, x_true_test),
                                                                batch_size)
  ...
  test_metric = functools.partial(
      ...
      y_true=y_test,
      pred_tensor=y_pred,
      dataset_init_op=test_init_op, print_size=False)
  if x_true_test is not None and y_true_test is not None:
      predictions = nn_model(true_test_x_batch, training=False)
      if regression:
          loss_fn, y_pred = penalized_mse_loss, predictions
      else:
          # Apply sigmoid transformation for binary classification
          loss_fn, y_pred = penalized_cross_entropy_loss, tf.nn.sigmoid(predictions)
      true_test_metric = functools.partial(
          evaluation_metric,
          y_true=y_true_test,
          pred_tensor=y_pred,
          dataset_init_op=true_test_init_op, print_size=False)
  else:
      true_test_metric = test_metric
  ...
  eval_metric_scores = {'test': test_metric, 'train': train_metric, 'true_test': true_test_metric}
