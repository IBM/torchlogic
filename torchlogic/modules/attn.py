...
from ..nn import (Predicates, AttentionLukasiewiczChannelAndBlock,
                  AttentionLukasiewiczChannelOrBlock)


class AttentionNRNModule(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            attn_emb_dim (int): Embedding dimension for latent variational space.
        """
        super(AttentionNRNModule, self).__init__()
        ...
        if self.normal_form == 'dnf':
            input_layer = AttentionLukasiewiczChannelAndBlock(
            ...
            input_layer = AttentionLukasiewiczChannelOrBlock(
                channels=output_size,
                in_features=input_size,
                out_features=layer_sizes[0],
                n_selected_features=input_size,
        ...
        for i in range(1, len(layer_sizes)):
            ...
            if i % 2 == 0:
                if self.normal_form == 'dnf':
                    internal_layer = AttentionLukasiewiczChannelAndBlock(
                        channels=output_size,
                    ...
                    internal_layer = AttentionLukasiewiczChannelOrBlock(
                        channels=output_size,
        ...
        if len(layer_sizes) % 2 == 0:
            if self.normal_form == 'dnf':
                self.output_layer = AttentionLukasiewiczChannelAndBlock(
                    channels=output_size,
                ...
                self.output_layer = AttentionLukasiewiczChannelOrBlock(
                    channels=output_size,
                    in_features=layer_sizes[-1],
                    out_features=1,
                    n_selected_features=layer_sizes[-1],
